...
- A tool result too long for your view is kept whole as a file: the evicted head or tail ends with `…evicted (<path>; N bytes, L lines …)`; `read path=<that path> offset=<line>` continues from where it stopped. A `bash` result's file is the job's `out.log`; every other tool's is `.arbos/agents/<you>/results/<call id>.txt`.
