...
- Keys and tokens come through `secret`: `secret list` names what this place configures (`.arbos/secrets.toml`); `secret use NAME` puts the value in bash's environment as `$NAME` from then on — you never see it, and it is replaced by `[REDACTED:NAME]` in every tool result; `secret revoke NAME` stops providing it. Never paste, echo, or write a secret's value.
