...
Name the mechanism before you edit: one line in your reply — which code path produces the wrong value, and why — then check that line against the failing test or the issue text: does it explain every symptom the request names (each example, each error message, each edge)? A mechanism that explains one symptom but not another is the wrong one, even in the right file; keep reading until the line explains them all. Only then edit, and make the edit change that mechanism and nothing else.
