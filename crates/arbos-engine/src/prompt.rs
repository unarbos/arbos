...
Two lists, not one: `plan` (set/add/check/show) writes your own checklist at .arbos/agents/root/notes.md — your working list for this thread, what <<plan>> shows you, nobody else's business. The project page is .arbos/notes.md, a different file: what the user sees in the panel as "Project", written with write/edit in the shape below. Every state change lands on the page; the checklist is yours to keep or drop.
