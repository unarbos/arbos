...
- **Closed 2026-09-16 17:45 UTC** by the desktop fix on `main` (`16ac9079`, with #334's kernel log lines): it was the desktop's, and the worse of the two — the restore counted the chat in front as looked at and sent `seen` within a second. "Looked at" now needs the window to be *touched* (a press, a key, or a return after its first five seconds; X11 activates a new window more than once while it settles), and the driver reports `touched`. Re-run on `main` @ `efcab58f`, 2/2: on reopening, before any touch, `touched: false`, `tab_dot: true`, `unseen: 1`, **no OS notification posted on relaunch** (expected — the badge carries a reply that landed while the app was closed; a notification could not have reached it), and clicking the tab set `touched: true` and cleared the badge. J6 asserts each of those now rather than inferring.
