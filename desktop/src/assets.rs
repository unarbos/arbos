...
///
/// Nothing at all where neither is there (a binary copied out of its
/// bundle away from the source tree), and whatever shows it has to hold
/// that case rather than stand an empty box where a picture goes.
pub fn mark() -> Option<PathBuf> {
    let exe = std::env::current_exe().ok()?;
    // `…/Arbos.app/Contents/MacOS/arbos` — the Makefile puts a small copy
    // of the icon beside the `.icns` that AppKit reads, because nothing here
    // can paint an `.icns`.
