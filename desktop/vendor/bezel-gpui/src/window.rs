...
/// ARBOS PATCH: where an identified element landed in the last rendered
/// frame. Recorded in [`crate::Interactivity::prepaint`] for every element
/// that has a [`GlobalElementId`], whether or not it inserted a hitbox, so an
/// out-of-process driver can find any `.id(...)`'d element by name and aim
/// synthetic input at it. Read back with [`Window::element_probes`].
#[derive(Clone, Debug)]
pub struct ElementProbe {
    /// The element's global id: every ancestor id joined by `.` in `Display`.
    pub id: GlobalElementId,
    /// Where it was laid out, in window pixels.
    pub bounds: Bounds<Pixels>,
    /// The clip it was drawn under. Bounds outside this are not visible.
    pub content_mask: ContentMask<Pixels>,
    /// Its hitbox, if it registered one. `None` means it takes no mouse input
    /// of its own (no listeners, no cursor style, no hover style).
...
pub(crate) struct Frame {
    ...
    /// ARBOS PATCH: see [`ElementProbe`]. Kept in step with `hitboxes` through
    /// `prepaint_index` / `reuse_prepaint` / `transact`.
    pub(crate) probes: Vec<ElementProbe>,
...
impl Window {
    ...
    pub(crate) fn prepaint_index(&self) -> PrepaintStateIndex {
        PrepaintStateIndex {
            ...
            probes_index: self.next_frame.probes.len(),
    ...
    pub(crate) fn reuse_prepaint(&mut self, range: Range<PrepaintStateIndex>) {
        ...
        self.next_frame.probes.extend(
            self.rendered_frame.probes[range.start.probes_index..range.end.probes_index]
                .iter()
                .cloned(),
        );
    ...
    pub fn transact<T, U>(&mut self, f: impl FnOnce(&mut Self) -> Result<T, U>) -> Result<T, U> {
        ...
        if result.is_err() {
            ...
            self.next_frame.probes.truncate(index.probes_index);
            self.next_frame
                .tooltip_requests
    ...
    /// ARBOS PATCH: record where an identified element landed this frame.
    /// Called by [`crate::Interactivity::prepaint`]; see [`ElementProbe`].
    ///
    /// This method should only be called as part of the prepaint phase of element drawing.
    pub fn insert_probe(
        &mut self,
        ...
        self.invalidator.debug_assert_prepaint();
        self.next_frame.probes.push(ElementProbe {
            id: id.clone(),
            bounds,
    ...
    /// ARBOS PATCH: every identified element in the last rendered frame, in
    /// prepaint order (parents before children, later siblings after earlier).
    pub fn element_probes(&self) -> &[ElementProbe] {
        &self.rendered_frame.probes
    }

    /// ARBOS PATCH: the hitboxes under `position` in the last rendered frame,
    /// topmost first, stopping at the first one that blocks the mouse. A
    /// hitbox in this list is what a real click at `position` would reach.
    pub fn hitboxes_at(&self, position: Point<Pixels>) -> Vec<HitboxId> {
        self.rendered_frame.hit_test(position).ids.into_vec()
    }
