...
- A control must live where nothing else builds, the loop included, and every labelled artefact must be able to prove its own label. A control binary kept at `target/release/` was silently rebuilt to the day's head by the same loop that was measuring against it, so a scenario "passed on the control" while running the fix — the separate-target-directory rule was obeyed and was not enough. Keep one immutable copy per commit, and check the label against the binary's own `--version` rather than trusting the path's name.
