> **RECOVERED from the repository mirror — believed complete, not byte-verified.** Source: `docs/design/cursor-vs-arbos-agent-model.md` in `unarbos/arbos` at commit `1374ed4` (2026-09-13 21:11 UTC), later than the store copy's last modification (2026-09-13 11:40 UTC). 29,463 bytes there against 29,481 in the store's last listing, so a little store-specific text may differ.
>
