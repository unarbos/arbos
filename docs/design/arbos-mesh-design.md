...
- On the machine, `online` and `offline_since_ms`: a machine whose last registrant left stays on the roster for seven days (memory only; a hub restart forgets sooner), `online: false`, with every project a kernel served there as a `live: false` row that keeps its face, place and `last_activity_ms`. An attach to it is refused with the offline word. So a phone can tell "this project is asleep" from "this machine was never here" (M-170). Absent `online` (an older hub) reads as true.
