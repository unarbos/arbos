...
7. The headline scenario replays Jacob's twelve-item acceptance benchmark (`scenarios/kickoff-session.json`) and records which items pass in `kickoff-history.jsonl`.
...
2. Who promotes drafts to `qa-NNN`? Recommend the QA worker, once per triage pass, with the coordinator's number sequence (numbers collided once: `qa-033`/`034`/`035` exist twice, from two writers — the second set is the ArbosLife/coordinator-side loop; keep both, prefix new ones with the writer).
