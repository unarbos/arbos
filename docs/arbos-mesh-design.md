...
**Installed with Jacob's yes, 2026-09-17 13:27 UTC — the mesh stack is under systemd user units.** `arbos-mesh.target` (enabled, wanted by `default.target`; linger `yes`) runs `arbos-hub`, `arbos-cf-tunnel`, `arbos-mesh-kernel` (demo), `arbos-mesh-feedback`, `arbos-mesh-worker`, `arbos-phone`; each was moved loop-first (tmux session killed, then the process, wait for exit, then the unit), then every unit was stopped and started through systemd once more and came back in 7 s and re-registered; one supervisor per process, checked by cgroup. **The two quick tunnels stay under tmux** (`hub-quick`, `phone-quick`): restarting `cloudflared` rotates their URLs, which would cut the voice gateway (attached via the phone kernel's quick URL) and the vault address for no gain; their units are enabled, so a boot starts them and writes fresh URLs to `public-url.txt`. **Not proved, because it needs a reboot and ArbosLife is not to be rebooted:** that `network-online.target` ordering is sufficient at boot, and that the quick-tunnel units come up and write their URLs. The hub's log is now `~/arbos-hub/logs/arbos-hub.service.log` (the old `hub/logs/hub.log` ends 13:26:51). **One thing went wrong:** `install.sh`'s stop pattern for the named tunnel, `cloudflared tunnel --no-autoupdate run`, also matched **Jacob's own `affine-tunnel` under his PM2**; it was killed at ~13:27:05 and PM2 restarted it at 13:27:29 (`restarts=1`, Cloudflare shows `affine-sn120` healthy, 4 connections). Roughly twenty seconds of his tunnel, self-healed, caused by a command-line pattern that was not an exact path — the rule this document exists to teach, broken by its author a third time. The pattern in `install.sh` is now the unit's own exact `ExecStart`.

**Hub rebuilt 2026-09-18 00:58 UTC from `main` `c3247332` (carries #538):** `GET /list` now has a per-project `last_activity_ms` (kernel reports at turn start and end, newest wins, roster broadcast when it moves; absent while zero). Deployed swap-first through `arbos-hub.service`; all seven registrants and the worker were back in 8 s. **The field shows only for kernels built with #538**: none of ArbosLife's kernels is (`0f2a8bc6`, `30eef166`, `efcab58f`, `cbbe9922`), and the dev channel's newest build (1733, `a26273c`) predates it, so their rows carry no value yet. Proved end to end with a scratch kernel built from `c3247332` registered as `podtest` through the tunnel: on registration its row read `last_activity_ms` = the value its disk knew (01:01:51); one turn later it read the turn's end (01:01:59), seen on the live roster four seconds after. The ArbosLife rows fill in as their kernels are updated to a build at or past `11a01d84`; nothing on the hub is waiting. `v0.2.0` was not published.

**Watched every half hour since 2026-09-17 18:22 UTC.** `mesh-health.sh` on the `store-watch` branch (fetched fresh each run by the mesh worker's timer at :22 and :52) checks, from outside ArbosLife: the six units active, `/healthz`, the roster carrying `demo`, `feedback`, `phone`, `subnet120` and the worker, no `binary_gone` on any registrant, no process on a deleted image, no double-served place, and the hub tunnel answering 200 from the internet. `MESH-OK` is silent; `MESH-FAULT` is re-checked after 20 s, then investigated and reported. Every run appends a line to `mesh/arboslife.jsonl` on the same branch, so the history outlives the machine that watches. It watches the mesh only: the `store-docs` mirror and the QA machine are QA's.

**The pod's deadline was real, and it is now 17 October.** At 13:25 UTC I wrote here that the pod does not expire; that was wrong, and the coordinator repeated it to Jacob before I caught it. I had read `termination_hours = None` — the *create-time input* — and missed the live field: `removal_scheduled_at = 2026-09-20T05:29:54Z`, set at rent time as a 7-day auto-termination, exactly as the voice worker's QA note of 13 Sep said (`internal/qa/inbox/2026-09-13-voice-server.md`, item 15: "Lium pod has a 7 day termination (2026-09-20 05:29 UTC)"). Every later mention — the coordinator's `notes.md` ("ends 09-20"), this document — descends from that correct line. With Jacob's choice C, at 13:33 UTC `POST /api/pods/ccb1295f…/schedule-removal` moved the termination to **2026-10-17T05:29:00Z** (HTTP 200, re-read from the pod record). What that commits us to: nothing beyond the running rate, $1.19/h by the second (about $860 for the 30 days) from the account balance, which covers it many times over; no new instance, no reprovisioning, nothing of Jacob's beyond the Lium API key already in the vault. The auto-termination is kept rather than removed so that forgetting still costs at most a month; one call moves it again (`schedule-removal` with a new date) or removes it (`DELETE` on the same path). Lesson for the record: when a provider has both an input field and a state field, read the state field, and when correcting a document, name the field you read.

**Staged for Jacob, 12:45 UTC — two things, one word each (superseded above):**
