// Package tui is the interactive Bubble Tea front-end for the pi coding agent.
package tui
...
func (m model) View() string {
	...
	b.WriteString(piStyle.Render("arbos") + dimStyle.Render("  (ctrl+c to quit)") + "\n\n")
...
// Run starts the interactive coding-agent session.
func Run(opts Options) error {
	dbPath := opts.DBPath
	if dbPath == "" {
		dbPath = piwire.DefaultDBPath()
	}
	approve := opts.Approve || os.Getenv("ARBOS_APPROVE") == "1"
