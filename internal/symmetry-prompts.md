...
- **Cycle 29 (opened 04:20 UTC)**: the kernel's `--version` as every run's first row (done, `cursor/cycle-29-aa39`); #385/#372 read from the desktop's side — nothing blanks, two suggestions filed (`features-inbox/2026-09-17-builds-list-and-binary-gone-in-the-desktop.md`; one of them, `unknown` shas, I had backwards — inferred, not traced; corrected, and #388 fixes both). #372 is closed unmerged; the stranger control is [#388](https://github.com/unarbos/arbos/pull/388) now; F-122's recurrence (log `turn_open` around the click); the remote track — install `main`'s kernel on ArbosLife through the app's own path and read its build line; the Cursor side of d11; light/dark side by side; R10's fold.
