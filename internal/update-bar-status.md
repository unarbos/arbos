...
Last checked 2026-09-22 14:17 UTC.

## Click Update. You will get build 2414. Jev is in it.

**2414** — signed, notarised, stapled, on the feed since 14:14 UTC.
...
New since 2410 (builds 2411–2414):

- **#815** — the conversation `+`
...
- **#812** — a mobile loop cycle

## Before that: 2405

New since 2402 (builds 2403–2405):

- **#811** — a mobile loop cycle

## Before that: 2402

New since 2398 (builds 2399–2402):

- **#810** — a mobile loop cycle

## Before that: 2398

New since 2394 (builds 2395–2398):

- **#808** — a mobile loop cycle
- **#809** — a dev-channel run stays for its own CI sibling, so a green
  arriving moments after a red one cannot sit unpublished

## Before that: 2394

New since 2391 (builds 2392–2394):

- **#807** — a mobile loop cycle

## Before that: 2391

New since 2388 (builds 2389–2391):

- **#806** — a mobile loop cycle

## Before that: 2388

New since 2385 (builds 2386–2388):

- **#805** — a mobile loop cycle

## Before that: 2385

New since 2382 (builds 2383–2385):

- **#804** — a mobile loop cycle
...
- **#799** — a mobile loop cycle

## Before that: 2367

New since 2359 (builds 2360–2367):

- **#798** — a mobile loop cycle

## Before that: 2359

New since 2353 (builds 2354–2359):

- **#797** — a mobile loop cycle

## Before that: 2353

New since 2349 (builds 2350–2353):

- **#795**, **#796** — two mobile loop cycles

## Before that: 2349

New since 2342 (builds 2343–2349):

- **#793**, **#794** — two mobile loop cycles

## Before that: 2342

New since 2339 (builds 2340–2342):

- **#792** — a mobile loop cycle

## Before that: 2339

New since 2330 (builds 2331–2339):

- **#789**, **#790**, **#791** — three mobile loop cycles

## Before that: 2330

New since 2326 (builds 2327–2330):

- **#788** — a mobile loop cycle

## Before that: 2326

New since 2322 (builds 2323–2326):

- **#786** — a mobile loop cycle
- **#787** — spawn, wait, stop

## Before that: 2322

New since 2319 (builds 2320–2322):

- **#785** — an await-loop fix

## Before that: 2319

New since 2317 (builds 2318–2319):

- **#784** — a mobile loop cycle

## Before that: 2317

New since 2312 (builds 2313–2317):

- **#782**, **#783** — two mobile loop cycles
...
- **#777** — several-workers opens a worker this run finished
- **#776** — a fault whose fix is unmerged is still reported every run
- **#779** — the two journey steps that were scored by eye are now read

## Before that: 2301

New since 2297 (builds 2298–2301):

- **#774** — three loop scenarios were installing yesterday's build

## Before that: 2297

New since 2294 (builds 2295–2297):

- **#772** — the call says "Hi." the moment the Live session is up

## Before that: 2294

New since 2288 (builds 2289–2294):

- **#770** — the names check said "the call" and meant three of its four
  controls
- **#771** — the dev channel asks again on a ten-minute timer, so a green
  whose own CI event never arrived cannot sit unpublished

## Before that: 2288

New since 2283 (builds 2284–2288):

- **#767** — the style pair is a check, not a ritual
- **#769** — voice: the project chat shows tool and agent rows on a call

## Before that: 2283

New since 2277 (builds 2278–2283):

- **#765** — a run says which coverage rows it exercised
- **#766** — the orb has no colour for thinking

## Before that: 2277

New since 2274 (builds 2275–2277):

- **#763** — what a returning user sees when the project moved on

## Before that: 2274

New since 2271 (builds 2272–2274):

- **#761** — a check dead for sixty-five cycles, and a helper that was
  never sourced
...
- **#749** — a read-only file is refused with the path rather than failing
  obscurely

## Before that: 2245

New since 2242 (builds 2243–2245):

- **#750** — the sweep of twenty-four, and the scene is kept when the
  separator fires
...
- **#744** — "already behaves as asked" means the asked output is present

## Before that: 2228

New since 2226 (builds 2227–2228):

- **#743** — a page of a large file streams, and a whole read over 64 MB
...
- **#739** — a reproduction that moves the working tree is handled as one

## Before that: 2218

New since 2216 (builds 2217–2218):

- **#738** — a chat-model fallback holds for the turn, so it does not flip
  back mid-answer
...
- **#726**, **#729** — a build-products rule tried and then withdrawn when
  it did not reproduce
- **#728** — the worker-chat style pair in numbers
...
- **#718** — `call-menu.sh` checks the call's menu
...
- **#713** — two verdicts that claimed what they had only printed
- **#714** — an empty `agent.md` is not an agent with every default
...
- **#708** — a `sleep` in a `for` loop over a literal list is that many
  passes, not a poll
- **#709** — two end-to-end tests wait for the worker's report

## Before that: 2150

New since 2146 (builds 2147–2150):

- **#706** — the sweep runs the eight checks that were sitting out
- **#707** — mobile: the projects-list row's status line is one VoiceOver
  element

## Before that: 2146

New since 2143 (builds 2144–2146):

- **#705** — settings-and-a-bad-token reads the settings sheet's numbers
...
- **#702** — `orb-phases.sh` reads the orb's colours
...
- **#700** — `list-faces.sh` checks the faces, which only pixels can see
...
- **#698** — a committed check for what the loop had been checking by hand
...
- **#692** — Terminal and Browser open promptly; the terminal pane reads
  the window's own size
- **#693** — mobile: the chat does not say "Working Thinking"
...
- **#677** — barge-in is a committed check, and it reads the reason the app
  prints
