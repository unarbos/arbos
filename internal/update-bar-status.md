...
Last checked 2026-09-18 00:10 UTC.

## Click Update. You will get build 1718.

**1718** — signed, notarised, stapled, on the feed since 00:07 UTC.

New since 1688, and all three of the ones that were being waited for:

- **#469** — a tab that never attached is asked for feedback over ssh,
  through the far side's own feedback CLI, and the sheet says which door
  the bundle came through
...
Still in it from earlier: **#490** voice lines in the project chat,
**#500** GPT-Live sees the project and live workers, **#501** Live voice
rows stay in the attached chat, **#499** a store is a folder, **#518**–
**#522**.
...
| | |
|---|---|
| Signed | `Developer ID Application: Jacob Steeves` — the build records `0.2.0 (1718, 869eb0511492) signed by` it |
...
| Bundle | `Info.plist` reads `0.2.0 build 1718` |
| Feed | every download it names is really on the tag |

The ticket was read out of `Arbos-0.2.0-1718-macos-arm64.zip` as
...
1616, 1624, 1657, 1662, 1674, 1688, 1718 — every twenty to forty minutes,
...
merging pauses long enough for a tip to settle — which is exactly what
happened here: `main` sat on `869eb051` for about fifteen minutes and the
publish went straight through.
