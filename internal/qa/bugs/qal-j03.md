...
Not started. Regression check: `journey-linux` J6 (`notify_before_click` must show `tab_dot: true, unseen ≥ 1` after a relaunch with a reply landed while closed), plus a kernel e2e: record a notification with no client attached, attach, expect a `notify` frame with `replayed: true` and the id, and no `seen` advance until a `seen` frame is sent.
