...
[#340](https://github.com/unarbos/arbos/pull/340) (features agent, 2026-09-16 17:50 UTC). The invariant first: after an output-owed reminder, a bash `mv`/`rm`/`unlink`/`git mv`/`git rm` of a file the turn wrote (or named like an owed path) is refused with the reason. Then the reminder accepts a file of the owed name written this turn wherever the task put it, and its words say never move or delete to match the brief. Then the contracts and the spawn tool's `output` field say `Output` is for artefacts agents read, and a user's project file goes in the task. `output_owed_e2e` covers both the regression check named above (no nudge, `CHANGELOG.md` still at the root) and the refused move. `journey-linux` J7 `changelog: true` should hold once #340 is on `main`. Regression check: `journey-linux` J7 (`CHANGELOG.md` at the project root after the run, `changelog: true`) and a kernel e2e: a brief with `output: .arbos/docs/X.md` and a task that writes `X.md` in the project must end without the "not written yet" nudge, and `X.md` must still be in the project at turn end.
