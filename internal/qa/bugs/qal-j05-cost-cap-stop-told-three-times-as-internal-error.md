...
- **Closed 2026-09-16 21:30 UTC** by #349 (`main` @ `0c82887b`): the cap is a place setting (`[spend] turn_cap_usd`), and a stop at it reads as the rule working — one plain notice, *"Stopped at the per-turn cap: this turn spent $0.0027 on model calls, over the $0.0001 you allow for one turn. What is in the working tree stays. A new message starts a fresh budget; to allow more per turn …"*, not marked failed, no `interrupted` beside it. `cp-01` and `cp-02` green on that build (cp-02: one notice item beside the tool card that legitimately ran, no "Internal error", no apology, driver answered in 96 ms).
