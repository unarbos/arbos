...
```
    ...
    20|The branch's set is a subset of the store's, so nothing would have been lost for ever — but the published record of 130 drafts would have gone, and the next `publish.sh pull` does not bring bug files back (it copies `inbox/` only), so no machine would have restored them.
