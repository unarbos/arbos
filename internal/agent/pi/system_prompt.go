...
func BuildSystemPrompt(opts PromptOptions) string {
	...
	// Git authorship: never attribute a commit to someone you aren't sure is
	// driving this session. The repo's user.name/user.email belong to whoever
	// configured the checkout, which may not be the current operator.
	add("When you create a git commit on the user's behalf, do not silently attribute it to an identity you are not sure of. If the operator driving this session is not clearly established (no authoritative speaker, and the repo's configured user.name/user.email may belong to someone else), ask who the commit author should be — or set an explicit author you can justify — rather than letting git record the inherited identity. Never attribute a commit to a person you are not sure is the one driving this session.")
