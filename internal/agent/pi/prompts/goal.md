...
**Finish properly.** For an in-turn goal, done means: definition of done holds, the tree builds, tests pass, and nothing is left half-wired. For a beyond-turn goal, finishing this turn means: the nodes exist and render armed, any step due now already ran, and the pipeline ends in a node that reports completion (a plain do:{notify} sibling, or a when:{onDeps} agent finisher) — the goal itself completes when the pipeline does. Then report concisely:
