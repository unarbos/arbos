...
## Fifth episode, 2026-09-17 00:37:11Z → 00:45:35Z — a loss; restored 00:48–00:56Z from `dff30aa7` (00:37:10Z)

Found by this worker's own write failing (`cp` into `docs/acceptance-journeys.md`: no such directory) at 00:45:3xZ, eight minutes after a clean push. Six reads over 00:45:41–00:47:44Z identical: `docs/` absent, `internal/mirror-docs.sh` absent, `internal/parity/` empty, 70 of 218 bug files present, `notes.md` and `media/desktop-feedback/` intact. Against `dff30aa7`: **215 files** — all 22 `docs/`, 21 `internal/features-inbox/`, all 15 `internal/parity/`, 148 bug files, 4 `internal/qa/`, `mirror-docs.sh` + the three `desktop-feedback-*.md`, one `internal/mobile/`. **Window 00:37:11Z → 00:45:35Z.** Restored tool first, then the rest (`git archive` → `~/arbos-qa/state/mirror-restore/20260917T0046Z-episode5/` → `cp`), 00:48:05–00:55:49Z, nothing still missing. The timer's 00:52 pass **refused** (shrink gate, mid-restore) — the alarm working. Post-restore pass green: `6badc665`, 00:59:26Z. Owners who wrote into a missing path between 00:37:11Z and 00:45:35Z should re-check; that is an eight-minute window, and the store's newest content otherwise survived in the snapshot.

**The subset fact, refined by this episode.** It is not one fixed set — it is *monotone*: every file lost at 22:54 was lost again at 00:45 (115 of 115), plus 33 more; every file lost at 14:23 was lost at 22:54 (99 of 99). And it is selective by *kind of file*, not by folder alone: in `internal/qa/bugs/` the 148 lost are all the loop's auto-drafted, hash-named files (`bd81b4e3cf.md` …); none of the 70 human-named `qa-*`, `ui-*`, `qal-*`, `store-*` files has been lost in any episode, though they sit in the same directory. `notes.md`, `media/`, `internal/qa/` tooling and `rollouts/`, `internal/voice/`, `internal/ui-research/` have survived every time; `docs/` whole, `internal/parity/`, `internal/features-inbox/` and `mirror-docs.sh` have gone every time since 12:24. Whatever selects, it selects the same way each time and takes more each time. Five episodes in seventeen hours; the last two eight and thirty-seven minutes after a clean snapshot.

## The report for the store's engineers

`docs/store-fault-report-2026-09-17.md` — the five episodes with their windows, what is taken and what never is, the monotone property, the evidence for service rather than client, the cost of recovery, and the one thing we cannot see (the server-side journal). Written 2026-09-17 01:10 UTC for Jacob to forward without editing; this record is its primary source.

## Sixth episode, 2026-09-17 03:23:40Z → ~03:27Z — watched happening; restored 03:29–03:35Z from `7d283951`

Found by this worker's own write failing again (`cp` of `docs/acceptance-journeys.md`: no such file, 03:26:4xZ), three minutes after a clean push (`7d283951`, 03:23:40Z, 24 docs). This time the reads caught the deletion **in progress**: 03:26:58 `mirror-docs.sh` present, `features-inbox/` 3, `parity/` 15; 03:27:16 `mirror-docs.sh` gone, `features-inbox/` 0; 03:27:29 `parity/` 1; by 03:28:57 stable at `parity/` 0, `internal/` 19 entries, 133 of 222 bug files. Counts fell monotonically between reads seconds apart and did not come back — a deletion sweeping the tree over about a minute, not a partial listing. Against `7d283951`: **174 files** — 23 `docs/`, 31 `internal/features-inbox/`, 15 `internal/parity/`, 89 bug files (the hash-named ones again), 11 top-level `internal/`, 4 `internal/qa/`, 1 `internal/mobile/`. Restored tool first, then the rest, 03:29:47–03:34:53Z; nothing still missing. One self-inflicted note: my 03:26 write had already recreated `docs/acceptance-journeys.md` from a local copy — the copy was current (identical to the 02:45 snapshot's version), so nothing was lost, but writing a document into a store mid-episode is not something to repeat; the procedure gains a line: *if a write fails with "no such directory", stop writing and read*. Window **03:23:40Z → 03:27:29Z** (the last falling read). Sixth in twenty hours; the intervals since the 22:37 episode: 2 h 0 m, 2 h 46 m.

## Per-client fault, 2026-09-17 from ~05:35Z — recorded live

The benchmark loop's VM saw this store list empty and refuse writes from about 05:35Z; the coordinator wrote `notes.md` throughout (mtime 05:56:56Z); this VM read `docs/` (25), `internal/mirror-docs.sh`, 234 bug files three times at 05:57:33–05:57:43Z and completed a write/read/delete probe at 05:57:48Z; the mirror pushed whole snapshots at 05:23:42Z and 05:41:09Z inside the other client's blackout. Not a deletion, not a settling listing: one client's session answered from nothing while others were served. **The mirror alarm did not fire because it could not:** it runs on this client, and this client's view was whole. Added to `docs/store-fault-report-2026-09-17.md` as its own section ("A third phenomenon"). Two changes to our side follow: a second reader on another machine would be needed to see this class at all; and, borrowed from the benchmark loop, when a write into the store fails the loop should stage the cycle's output locally and apply it when the mount returns rather than lose it — noted for the next cycle script change.
