...
func PromptInfo() codingspec.ToolPromptInfo {
	return codingspec.ToolPromptInfo{
		...
		Guidelines: []string{
			...
			"do:{shell} and do:{notify} cost no model turn — the kernel runs them. A reminder or completion ping is do:{notify} (e.g. when:{after:\"30m\"} do:{notify:\"stand up\"}); a step whose entire effect is captured by a fixed command (build, test, push, a commit with a fixed message) is do:{shell}. A step that must compose text or weigh a result each firing — a commit whose message should describe the work, a triage, a summary — needs judgment: make it an agent task (do omitted), not a cleverer command.",
			...
			"Never embed another LLM CLI (claude, llm, …) in a do:{shell} command to smuggle judgment into a pipeline. The agent executor IS the judgment step: an agent-task firing is a fresh session of you, carrying the plan forest, memory, and the node's attempt history — a foreign CLI has none of that and its turn is no cheaper. A node that needs generated text or a weighed decision is mis-typed as shell; retype it, don't re-engineer the command.",
			...
			"Iterated goals — \"each time I'm called…\", \"count to N, one per invocation\", \"keep doing X until Y\" — compile to the plan, never to ad-hoc state files or a turn held open: a bounded sequence is sibling nodes the kernel chains in order (mechanical steps as do:{shell} or do:{notify}; steps needing judgment as agent tasks with when:{onDeps} — each firing does its one step and finishes its node, which summons the next); an open-ended or clocked iteration is one when:{every} agent node that carries progress in its last: outcome and cancels itself when the stop condition holds.",
			"A recurring pipeline that ENDS in judgment compiles to a gate, not a smarter script: put the mechanical prep in when.condition on the poll cadence and leave do as an agent task — e.g. hourly \"pull, build, commit with a message describing the work\" is when:{every:\"1h\", condition:\"git pull --ff-only && go build ./... && test -n \\\"$(git status --porcelain)\\\"\"} with do omitted: the kernel polls for free every hour and wakes you only when the build is green and there is work to commit; you then write the message, commit, push, and record the recurrence. For one-shot shapes, mechanical do:{shell} siblings followed by a when:{onDeps} agent node do the same — prep with no model turn, then a summons for the judgment finish.",
			"Success is silent unless asked: to hear about completion, end the pipeline with a final reporter node — a plain do:{notify} sibling for a fixed message (mechanical nodes fire when their earlier siblings finish; onDeps is rejected on them), or a when:{onDeps} agent task to compose one. A ready agent node with no trigger never fires on its own; never promise a completion ping without one of these.",
