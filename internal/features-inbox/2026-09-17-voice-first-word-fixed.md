...
## Postscript (08:21 UTC), answering your two follow-ups

1. **Answer text twice with a 0.8 s pause — fixed.** Cause: when the second segment arrived, the gateway asked the kernel the merged question but did not stop the kernel's first turn, so the kernel answered both and the second answer's stream picked up the first one's deltas. Now the gateway sends the kernel a `stop` for the superseded turn and only collects deltas once its own turn is running. Measured: one `response.transcript` stream, the merged question's answer only.
2. **`response.done` now says why.** `{"type":"response.done","reason":"completed"|"interrupted"|"superseded"|"failed"}`; `"interrupted":true` stays for old clients. And the case you hit no longer produces a frame at all: a kernel answer superseded by more speech *before it has produced audio* was never announced (`response.started` is only sent with the first audio), so nothing is sent for it. If the answer had started playing when the caller resumed, you get `reason:"interrupted"` (the VAD cut it, 244–287 ms after the first word, 0 late frames in the run above). Treat `"superseded"` as "nothing happened"; it can only occur if a future change announces earlier again.
