...
| notifications (`notify`, push) | 15 (banner on `pod`), 19 (push check via `GET /push` + `/push/test`, U until the hub has #333 and Jacob's key) | simulator, `simctl push` with the hub's payload | banner, badge, tap-to-project, away card, seen both ways all verified; real pushes start when Jacob's APNs key + capability land (no release needed) |
