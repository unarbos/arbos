...
| M-88 | 09-16 | loop (cycle 13) | list | once, after a cold start with the hub unreachable, the list fell from four cached rows to a single `pod · Idle` row and stayed there after the link returned; a second identical run kept all four rows ("Off", then "Idle") | open — a failed roster refresh must never shrink the list; find the path that replaced the cache |
