...
export function PromptEditor({
  ...
  useEffect(() => {
    if (!active || state.phase !== "ready" || dirty) return;
    let stop = false;
    const check = () => {
      fetchFile(surface.path)
        .then((info) => {
  ...
  }, [active, state.phase, dirty, surface.path]);
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col">
      <div className="flex h-9 shrink-0 select-none items-center gap-2 border-b border-line/70 px-3">
        <span className="min-w-0 truncate font-mono text-[12.5px] text-accent">
          {surfaceTitle(surface)}
        </span>
        <span className="min-w-0 flex-1 truncate font-mono text-[11px] text-faint">
          {surface.path}
        </span>
        {saved === null && state.phase === "ready" && (
        ...
        <button
          type="button"
          onClick={save}
          disabled={!dirty || saving || state.phase !== "ready"}
          title="Save (⌘S)"
          className={`flex h-6 shrink-0 cursor-pointer items-center gap-1.5 rounded-md px-2.5 text-[11.5px] transition-colors disabled:cursor-default ${
            dirty && state.phase === "ready"
        ...
        >
          {saving ? (
            ...
            !dirty && state.phase === "ready" && <Check size={11} />
          )}
          {dirty ? "Save" : "Saved"}
        </button>
      </div>
      ...
      {state.phase === "ready" && (
        <>
          ...
          />
          <div className="flex h-7 shrink-0 items-center gap-2 border-t border-line/70 px-3 text-[11px] text-faint">
            {saveError ? (
              <span className="truncate text-red">{saveError}</span>
            ) : (
