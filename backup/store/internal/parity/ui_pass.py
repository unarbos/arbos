...
Turn results.json into the Markdown table with ui_pass_report.py.
...
class Pass:
    ...
    def notice_check(self, element: str, screen: str, action: str, expected: str) -> None:
        ...
        tail = [it for it in items[-4:] if it.get("kind") == "notice"]
    ...
    def phase_tabs(self) -> None:
        ...
        try:
            ...
            self.send("Reply with exactly: notification check done.")
            time.sleep(1.5); self.app.click("tab-0"); time.sleep(0.5)
            self.wait_idle(120); time.sleep(3)
            def root_of(st):
                return next((c for pr in st["projects"] if pr["path"].rstrip("/") == str(PROJ).rstrip("/") for c in pr["sessions"] if c.get("parent") is None), {})
            away = root_of(self.state())
            self.record("notify-unseen", sc, "reply lands while the Home tab is in front", "the project's root chat holds 1+ unseen notification (tab dot)",
                        f"unseen={away.get('unseen')} seen_through={away.get('seen_through')}", "pass" if (away.get("unseen") or 0) >= 1 else "fail", self.still("notify-unseen"))
                ...
                for el in self.ids():
                    ...
                    if short == "bionic-reading":
                        ...
                        def prose_pixels():
                            self.app.use_window("main")
                            try:
                                # An agent paragraph: the user's own prompt
                                # is never weighted, so it would show nothing.
                                st = self.state()
                                chat = next((c for c in sessions(st) if c.get("id") == st.get("active_session")), None)
                                agent_ix = {i for i, it in enumerate((chat or {}).get("items", [])) if it.get("kind") == "agent"}
                                wid = subprocess.run(["xdotool", "search", "--name", "^Arbos$"], capture_output=True, text=True, env=ENV).stdout.split()
                                if not wid:
                                    return None
                                geo = dict(line.split("=", 1) for line in subprocess.run(["xdotool", "getwindowgeometry", "--shell", wid[0]], capture_output=True, text=True, env=ENV).stdout.split())
                                ox, oy, wh = int(geo.get("X", 0)), int(geo.get("Y", 0)), int(geo.get("HEIGHT", 10_000))
                                # The largest agent paragraph that is actually on
                                # screen: the transcript scrolls, and a box above
                                # the fold would crop the background twice.
                                boxes = []
                                for el in self.ids("transcript-prose-*"):
                                    if int(el.rsplit("-", 1)[-1]) not in agent_ix:
                                        continue
                                    b = self.app.find(el)
                                    if b["y"] >= 0 and b["y"] + b["h"] <= wh and b["w"] > 0 and b["h"] > 0:
                                        boxes.append(b)
                                if not boxes:
                                    return None
                                box = max(boxes, key=lambda b: b["w"] * b["h"])
                                # The toggle flips a global the main window
                                # only repaints on its next event: give it one.
                                ...
                                x, y, w, h = ox + int(box["x"]), oy + int(box["y"]), int(box["w"]), int(box["h"])
                                return Image.open(shot).convert("L").crop((x, y, x + w, y + h))
                            finally:
                                self.app.use_window("settings")
                        plain = prose_pixels()
                        ...
                        if plain is not None and weighted is not None and plain.size == weighted.size:
                            ...
                            self.record("weight-visible", "settings-window", "compare the first prose paragraph before/after the toggle", "the rig sees the weight change (> 1 % of the paragraph's pixels differ)",
                                        f"{changed}/{total} pixels differ ({100.0 * changed / total:.1f} %)", "pass" if changed > 0.01 * total else "fail", self.still("weight-visible"))
                        else:
...
def main() -> int:
    ...
    drv = load_driver(Path(args.driver_py) if args.driver_py else PARITY / "arbosdriver.py")
